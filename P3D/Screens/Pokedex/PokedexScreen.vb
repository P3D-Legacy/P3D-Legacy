Public Class PokedexSelectScreen
    ...
    Private Sub DrawCursor()
        Dim cPosition As Vector2 = New Vector2(448, 96 + Me.Cursor * 96 - 42)
    ...
    Public Overrides Sub Draw()
        ...
        For i = Scroll To Scroll + 5
            If i <= Me.HabitatList.Count - 1 Then
                ...
                Core.SpriteBatch.DrawString(FontManager.MainFont, HabitatList(i).Name, New Vector2(200, 128 + p * 96), Color.Black, 0.0F, Vector2.Zero, 1.0F, SpriteEffects.None, 0.0F)

                Dim t As String = HabitatList(i).PokemonCaught.ToString() & "/" & HabitatList(i).PokemonList.Count
                Core.SpriteBatch.DrawString(FontManager.MainFont, t, New Vector2(680 - CSng((FontManager.MainFont.MeasureString(t).X * 1.0F) / 2.0F), 116 + p * 96), Color.Black, 0.0F, Vector2.Zero, 1.0F, SpriteEffects.None, 0.0F)

                Dim progressTexture As Texture2D = Me.HabitatList(i).ProgressTexture
                If Not progressTexture Is Nothing Then
                    Core.SpriteBatch.Draw(progressTexture, New Rectangle(CInt(650 - CSng((FontManager.MainFont.MeasureString(t).X * 1.0F) / 2.0F)), 121 + p * 96, 20, 20), Color.White)
                End If
            End If
        Next
